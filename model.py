...
class DESDNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 这里的参数分别表示输入通道数，输出通道数，卷积核大小，步长，padding大小 padding表示在图像周围填充0的层数
        ...
        up_down1_dst1 = self.conv1x1_ReLU_down1(
            torch.cat((down1_dst1, down2_dst1_1, down3_dst1_1, down4_dst1_1), 1)
        )
        up_down1_dst2 = self.conv1x1_ReLU_down1(
            torch.cat((down1_dst2, down2_dst2_1, down3_dst2_1, down4_dst2_1), 1)
        )
        ...
        up_down0_dst1 = self.conv1x1_ReLU_down0(
            torch.cat(
                (down0_dst1, down1_dst1_0, down2_dst1_0, down3_dst1_0, down4_dst1_0), 1
            )
        )
        up_down0_dst2 = self.conv1x1_ReLU_down0(
            torch.cat(
                (down0_dst2, down1_dst2_0, down2_dst2_0, down3_dst2_0, down4_dst2_0), 1
            )
        )
        up_down0_shad = self.conv1x1_ReLU_down0(
            torch.cat(
                (down0_shad, down1_shad_0, down2_shad_0, down3_shad_0, down4_shad_0), 1
            )
        )
