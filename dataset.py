...
'''
def make_dataset(root):
    img_list = [os.path.splitext(f)[0] for f in os.listdir("/root/du/ISTD_Dataset/ISTD_Dataset/train/train_A/") if f.endswith('.png')]
    return [
        (os.path.join("/root/du/ISTD_Dataset/ISTD_Dataset/train/train_A/", img_name + '.png'), 
        os.path.join("/root/du/ISTD_Dataset/ISTD_Dataset/train/train_B/", img_name + '.png'),
        os.path.join(root, 'fuse_dst1', img_name + '.png'),os.path.join(root, 'fuse_dst2', img_name + '.png'))
        for img_name in img_list]
